...
class LTBanner: UIView {
    ...
    override init(frame: CGRect) {
        ...
        control.currentPageIndicatorTintColor = LTTheme.select
        control.pageIndicatorTintColor = RGB(222, 222, 222)
    }
...
extension LTBanner: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cells = cell as? BannerCell, let aDatas = datas {
            cells.imageView.image = placeholder
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == banner {
            ...
            let index = Int((scrollView.contentOffset.x+banner.bounds.width/2)/banner.bounds.width)-1
            if index == imageDatas.count-2 {
                control.currentPage = 0
            } else if index == -1 {
                control.currentPage = imageDatas.count-3
            } else {
...
class BannerCell: UICollectionViewCell {
    ...
    func upDataModel() {
        ...
        if let imageUrl = data.image.imageUrl {
            imageView.kf.setImage(with: imageUrl.url, placeholder: imageView.image)
        }
